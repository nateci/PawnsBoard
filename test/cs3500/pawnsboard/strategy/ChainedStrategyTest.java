...
public class ChainedStrategyTest {
  ...
  public void testFallbackToSecondStrategy() {
    // Strategy 1 returns no moves, Strategy 2 returns one move
    Strategy strategy1 = (model, color) -> Collections.emptyList();
    Strategy strategy2 = (model, color) ->
            Collections.singletonList(new Move(1, 1, 0));
  ...
  public void testFirstStrategyTakesPriority() {
    Strategy first = (model, color) ->
            Collections.singletonList(new Move(0, 0, 1));
    Strategy second = (model, color) ->
            Collections.singletonList(new Move(2, 2, 0));
